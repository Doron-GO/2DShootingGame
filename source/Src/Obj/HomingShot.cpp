...
void HomingShot::Shoot(Vector2 pos)
{
	...
	if (player_->life_ > 0)
	{
		if (pad.IsPadBtnNew(player_->padNumber_, InputManager::JOYPAD_BTN::L_1))//ÉvÉåÉCÉÑÅ[ÇPÇÃíeî≠éÀ
		{
			int count = 0;
			if (shotframe % 8 == 0)//àÍî≠Ç∏Ç¬î≠éÀ
			{
				for (auto& b : bullets) {
					if (!b.isActive) {
						PlaySoundMem(soundH, DX_PLAYTYPE_BACK);
						b.pos = player_->GetPlayerPos();//åªç›ín
						HomingPos(true);
						b.vel = homingPos - player_->GetPlayerPos(); //á@
						b.vel.Normalize();//áAê≥ãKâªÇ≈í∑Ç≥ÇÇPÇ…
						b.vel *= speed;	//áAÉXÉsÅ[ÉhÇä|ÇØÇÈ
						b.isActive = true;		
						count++;
					}
					else {
	...
	for (auto& b : bullets) {
		...
		//7å¬ÇΩÇ‹Ç¡ÇΩÇÁàÍî‘å√Ç¢Ç‡ÇÃÇéÃÇƒÇÈ
		if (b.history.size() > 5) {
			b.history.pop_back();
		}

		//è≠ÇµÇÃä‘ÇæÇØÉzÅ[É~ÉìÉOÇ∑ÇÈ
		if (b.count <= 40)
		{
			HomingPos(false);
